...
3. **Mechanical Evidence Axiom**: No assertion is accepted without fresh command execution returning Exit Code `0`. Never trust "should work", "looks good", or static assumptions. Executing `./.agentc-kit/cli/agentc verify` is MANDATORY immediately after modifying any code file.
...
- **Gate 1 (Contract)**: Canonical DTOs (`@repo/contracts`), DB migrations, `package.json` dependency lock + deterministic seed data. Gate 1 MUST lock framework dependencies before advancing to Gate 2.
...
4. **Mechanical Enforcement**: Run `./.agentc-kit/cli/agentc verify` after ANY code modification. Any failure emits Exit Code `1` and halts progression.
...
7. **Completion Signal**: Emit `<!-- GOAL_COMPLETE -->` ONLY when 100% of gates pass with Exit Code `0` from an IMMEDIATELY PRECEDING run of `./.agentc-kit/cli/agentc verify`, audit report shows zero violations, and zero zombie processes. Emitting completion without running `verify` in the current turn is strictly forbidden.
